...
/// This prevents metadata keys like __active_identity_id from breaking deserialization
pub fn process_raw_identity_map(val: Value) -> IdentityMap {
    let mut identity_map = HashMap::new();
    if let Some(obj) = val.as_object() {
        for (key, value) in obj {
            // Skip metadata keys used for app state
...
pub(crate) fn load_identity_map_internal(
    ...
    // get_network_file ensures filenames like .identity-testnet.json
...
/// INTERNAL: Handles map saving using a Generic Store
pub(crate) fn save_identity_map_internal(
    ...
    // Inject metadata for frontend discovery of the active identity
