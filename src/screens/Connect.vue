        ...
        <section class="flex items-center justify-center min-h-[calc(100vh-140px)] px-4">
            <div class="max-w-2xl w-full mx-auto space-y-8 border-2 border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl p-8 bg-white dark:bg-slate-900">
                ...
                <form @submit.prevent="handleConnect" class="space-y-6">
                    ...
                    />
                    <!-- Debug Output with Toggle -->
                    <div v-if="debugOutput" class="space-y-2">
                        <div class="flex justify-end">
                            <button
                                type="button"
                                @click="showDebug = !showDebug"
                                class="text-xs text-slate-400 hover:text-cyan-600 dark:hover:text-cyan-400 underline transition-colors"
                            >
                                {{ showDebug ? 'Hide Debug Info' : 'For Debug Purposes' }}
                            </button>
                        </div>
                        <div v-if="showDebug" class="p-4 bg-slate-100 dark:bg-slate-800 rounded-xl border border-slate-300 dark:border-slate-700">
                            <h3 class="text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Discovery Info</h3>
