    ...
    <main>
        ...
        <section class="bg-gray-50 dark:bg-slate-900 font-sans text-slate-900 dark:text-slate-200 min-h-screen border-2 border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl">
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
                <!-- (Template content identical to your version, no changes needed in HTML) -->
                <!-- ... -->
                <div class="space-y-12">
                    ...
                    <div v-else-if="identities.length === 0" class="text-center py-12">
                        ...
                        <p class="mt-2 text-slate-600 dark:text-slate-400">No identities were found. Connect or register a new one.</p>
                    </div>

                    <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div v-for="identity in identities" :key="identity.id || identity.identityId"
                            class="bg-white dark:bg-slate-800 rounded-xl border-2 transition-all duration-200 flex flex-col shadow-xl hover:shadow-2xl hover:-translate-y-1 border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 group"
                            ...
                            <div class="p-6 flex items-start gap-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p v-if="identity.dpnsUsername" class="text-slate-600 dark:text-slate-400 text-sm truncate">{{ identity.dpnsUsername }}</p>
                                    <p v-else class="text-slate-500 dark:text-slate-500 text-sm italic">No DPNS name</p>
                                </div>
                            ...
                            <div class="px-6 pb-6 space-y-4 flex-1">
                                <div>
                                    <label class="text-xs font-medium text-slate-600 dark:text-slate-400 mb-2 block">Key Status</label>
                                    <div class="space-y-2">
                                        <div v-for="key in identity.publicKeys?.slice(0, 3)" :key="key.id" class="flex items-center justify-between text-sm">
                                            <span class="font-medium truncate">{{ getPurposeLabel(key.purpose) }}</span>
                                            <span :class="getSecurityLevelClass(key.securityLevel)" class="px-2 py-1 text-xs rounded-full whitespace-nowrap">{{ getSecurityLevelLabel(key.securityLevel) }}</span>
                                        </div>
                                        <div v-if="identity.publicKeys && identity.publicKeys.length > 3" class="text-xs text-center text-slate-400">
                                            + {{ identity.publicKeys.length - 3 }} more keys
                                        </div>
                                        ...
                                        <div v-if="!hasTransferKeyForIdentity(identity)" class="mt-3 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800">
                                            <div class="flex items-center gap-2 text-amber-800 dark:text-amber-300">
                                                ...
                                                <span class="font-semibold text-sm">Missing TRANSFER</span>
                                            </div>
                                            <p class="text-amber-700 dark:text-amber-400 text-xs mt-1">Cannot send transactions</p>
                                            <RouterLink :to="`/identity/${identity.id || identity.identityId}/keys/add`" class="mt-2 block w-full text-center rounded-lg bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white py-2 px-4 text-xs font-semibold">Add Key</RouterLink>
                                        </div>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="relative">
                                        <input type="text" readonly :value="identity.id || identity.identityId" class="w-full bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-200 font-mono text-sm p-3 rounded-xl border border-slate-300 dark:border-slate-700 pr-12 shadow-sm focus:ring-2 focus:ring-cyan-400 focus:border-cyan-400 transition-all duration-200" />
                            ...
                            <div class="mt-auto p-6 border-t border-slate-200 dark:border-slate-700 space-y-3">
                                <RouterLink :to="`/identity/${identity.id || identity.identityId}/keys`" class="block w-full text-center rounded-xl bg-gradient-to-r from-slate-400 to-slate-500 dark:from-slate-500 dark:to-slate-600 hover:from-slate-300 hover:to-slate-400 dark:hover:from-slate-400 dark:hover:to-slate-500 text-white py-3 px-6 text-sm font-bold shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all duration-200">
                                    Manage Keys
                                </RouterLink>
                                <button v-if="(identity.id || identity.identityId) !== activeIdentityId" @click="handleSwitchIdentity(identity.id || identity.identityId || '')"
...
const { identityId: currentActiveId } = useIdentity()
...
const copyToClipboard = (text: string) => {
    navigator.clipboard.writeText(text)
        .then(() => triggerNotification('success', 'Identifier copied to clipboard'))
        .catch(_err => triggerNotification('error', 'Failed to copy to clipboard'))
}
...
const handleSwitchIdentity = async (id: string) => {
    try {
        ...
        // Use Store Action
        const result = await identityStore.switchIdentity(id)

        if (result.success) {
            activeIdentityId.value = id
            triggerNotification('success',`Switched to ${id.slice(0, 8)}...`)
            // Refresh local list to update "Active" pill immediately if needed,
            // though reactive state usually matches
        } else {
            throw new Error(result.error)
        }
    } catch (error: any) {
        console.error('Failed to switch identity:', error)
        triggerNotification('error', error.message || 'Failed to switch identity')
    } finally {
        loading.value = false
    }
}

// Load identities from Rust storage
const loadIdentitiesFromStorage = async () => {
    try {
        loading.value = true
        const network = await ensure()

        // Load persistend identities
        const store = await identityStore.loadDiscoveredIdentities(network)

        if (store && store.identities) {
            // Convert Map/Object to Array
            identities.value = Object.values(store.identities).map((rustId: any) => ({
                identityId: rustId.identityId || rustId.identity_id,
                identityIdx: rustId.identityIdx || rustId.identity_idx,
                dpnsUsername: rustId.dpnsUsername || rustId.dpns_username,
                balance: rustId.balance,
                revision: 0,
                publicKeys: [] // Details typically only fetched when active
            })).sort((a,b) => a.identityIdx - b.identityIdx)
        } else {
            identities.value = []
        }

        // Set active from global state
        if (currentActiveId.value) {
            activeIdentityId.value = currentActiveId.value
        }
    } catch (error) {
        console.error('Failed to load identities from storage:', error)
    } finally {
        loading.value = false
    }
}
...
onMounted(async () => {
    await loadIdentitiesFromStorage()
    window.addEventListener('identity:updated', loadIdentitiesFromStorage)
})

onUnmounted(() => {
    window.removeEventListener('identity:updated', loadIdentitiesFromStorage)
})
</script>
