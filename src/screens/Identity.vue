...
import type { DiscoveredIdentity, ConnectionResult } from '@/types/identity'
...
const handleSwitch = async (targetId: string) => {
    ...
    loading.value = true
    try {
        // cast to ConnectionResult to fix 'void' error if useConnect implementation is missing returns
        const result = await switchIdentity(targetId) as unknown as ConnectionResult
        if (result && result.success) {
            console.log(`Switched to ${targetId}`)
        } else {
            const error = (result && result.error) ? result.error : 'Unknown error'
            alert("Failed to switch: " + error)
        }
    } catch (e) {
        console.error(e)
