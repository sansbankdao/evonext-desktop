            ...
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
                <div class="space-y-12">
                                ...
                                <div v-if="showMissingKeys" class="mt-4 space-y-2">
                                    <div v-for="identity in missingTransferKeys" :key="identity.id" class="rounded-lg bg-amber-50 dark:bg-amber-900/20 p-3">
                                        <div class="flex items-center justify-between">
                                            <span class="font-medium text-amber-900 dark:text-amber-200">{{ identity.displayName || identity.username }}</span>
                                            <span class="text-xs font-mono text-amber-700 dark:text-amber-400 truncate ml-2">{{ identity.id.slice(0, 8) }}...{{ identity.id.slice(-8) }}</span>
                                        </div>
                    ...
                    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <!-- Use v-for to loop through actual identities -->
                        <div v-for="identity in identities" :key="identity.id"
                            ...
                            <div class="p-6 flex items-start gap-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h2 class="text-xl font-bold text-slate-900 dark:text-slate-100 truncate">{{ identity.displayName || identity.username || 'Unnamed Identity' }}</h2>
                                    <p class="text-slate-600 dark:text-slate-400 text-sm truncate">{{ identity.username || 'No DPNS name' }}</p>
                                </div>
                            ...
                            <div class="px-6 pb-6 space-y-4 flex-1">
                                <div>
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <div v-if="!hasTransferKey(identity)" class="mt-3 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800">
                                            ...
                                            <RouterLink :to="`/identity/${identity.id}/keys/add`"
                            ...
                            <div class="mt-auto p-6 border-t border-slate-200 dark:border-slate-700 space-y-3">
                                <RouterLink :to="`/identity/${identity.id}/keys`"
...
const missingTransferKeys = computed(() => {
    return identities.value.filter(identity => !hasTransferKey(identity))
})

// Helper functions
const getAvatar = (identity: Identity): string => {
    if (identity.avatarUrl) return identity.avatarUrl
    const name = identity.displayName || identity.username || identity.id
...
const getPurposeLabel = (purpose: number) => {
    switch(purpose) {
        ...
        case 3: return 'TRANSFER' // TRANSFER is also 3 in your JSON
...
const init = async () => {
    try {
        ...
        /* Set active identity from store if available */
        if (identityStore.identity?.id) {
            activeIdentityId.value = identityStore.identity.id
        }
        ...
        if (foundIdentities && foundIdentities.length > 0) {
            ...
            identities.value = foundIdentities.map((identity: any) => {
                // Try to get DPNS name if available
                const dpnsName = identity.username || identity.id

                return {
                    id: identity.id,
                    username: dpnsName,
                    displayName: dpnsName.split('.')[0] || 'Unnamed',
                    bio: `Identity with ${identity.publicKeys?.length || 0} registered key${identity.publicKeys?.length !== 1 ? 's' : ''}`,
            ...
            if (!activeIdentityId.value && identities.value.length > 0) {
                activeIdentityId.value = identities.value[0].id
            }
        } else {
