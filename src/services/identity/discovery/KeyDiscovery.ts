...
import type { DiscoveredIdentity, IIdentityActions, DiscoveryResult, DiscoveryOptions, IPublicKey, PurposeType, SecurityLevelType } from '@/types/identity'
  ...
  async discoverFromKey(
    ...
    try {
      ...
      if (result.success && data && typeof id === 'string') {
        ...
        return { success: true, identities: [discovered], identity: discovered } as DiscoveryResult
      }
