...
export class KeyDerivationService {
    ...
    static async deriveAllKeysFromSeed(
        ...
        maxIdentityIndex: number = 3, // Reduced default as requested
            ...
            for (let identityIdx = 0; identityIdx < maxIdentityIndex; identityIdx++) {
                ...
                for (const keyInfo of keyPurposes) {
                    ...
                    try {
                        ...
                        // Construct derivation path string manually for display
                        // DIP9 Standard: m/9'/<coin_type>/<identity_index>'/0/<key_index>
                        const path = `m/9'/${coinType}/${identityIdx}'/0/${keyInfo.index}`
                        keys.push({
                        ...
                        console.warn(`[KeyDerivation] Failed key ${keyInfo.index} idx ${identityIdx}`)
                    }
                }
                results.push({
